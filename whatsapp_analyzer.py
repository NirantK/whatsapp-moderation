...
class WhatsAppGroupAnalysis:
    ...
    def get_users_with_zero_messages(self) -> pd.DataFrame:
        ...
        users_with_messages = pd.merge(current_users_df, message_count_in_window, on="User", how="left").fillna(0)
        # Filter users with 0 messages
        users_with_zero_messages = users_with_messages[users_with_messages["Message_Count_In_Window"] == 0]
...
def analyze_single(input_path: Path, output: Optional[Path], window_days: int, exclude_contacts: bool):
    ...
    inactive_users = inactive_users.sort_values(by=["Total_Messages_Sent", "Joining_Date"], ascending=[True, True])
